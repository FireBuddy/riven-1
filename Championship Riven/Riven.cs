...
namespace Championship_Riven
{
    class Riven
    {
        ...
        {
            if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.LaneClear) && sender is Obj_AI_Turret && sender.Distance(Player.Instance) < 800 && sender.IsAlly)
            {
                ...
                if (!(args.Target is AIHeroClient))
                {
                    ...
                    if(Minion.Health > Player.Instance.TotalAttackDamage && (Minion.IsMelee && Minion.Health - sender.TotalAttackDamage * 1 <= 0) || (!Minion.IsMelee && Minion.Health - sender.TotalAttackDamage * 1.5 <= 0)  )
        ...
        {
        {
            ...
            if (args.SData.Name.ToLower().Contains(Riven.Q.Name.ToLower()))
            {
                ...
                Core.DelayAction(() =>
                {
                    if (!Player.Instance.IsRecalling() && CountQ <= 1)
                    {
