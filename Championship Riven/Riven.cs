            ...
            foreach (var Minion in Minions)
            {
                ...
                if(Q.IsReady())
                {
                    if(Minion.IsValidTarget(Q.Range * 2 + 125) && !Minion.IsDead )
                    {
                        ...
                        if(Player.Instance.IsFacing(Minion) && ObjectManager.Player.Position.Distance(Minion.ServerPosition) > 409 && Minion.Health - SpellQDamage(Minion, Minion.Health) * 2 <= 0)
                        {
                ...
                else if(E.IsReady() && CountQ => 2 && Minion.IsValidTarget(E.Range + 125) && ObjectManager.Player.Position.Distance(Minion.ServerPosition) > 300  && Minion.Health - SpellQDamage(Minion, Minion.Health) * 2 <= 0 )
                {
