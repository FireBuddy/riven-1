        ...
        private static void LastHit()
        {
            ...
            foreach (var Minion in Minions)
            {
                if(Q.IsReady())
                {
                    ...
                    {
                        if(ObjectManager.Player.Position.Distance(Minion.ServerPosition) > 250 && Orbwalker.OnPreAttack.NetworkId != Minion.NetworkId && Minion.Health - SpellQDamage(Minion, Minion.Health) * 2 <= 0)
                        {
