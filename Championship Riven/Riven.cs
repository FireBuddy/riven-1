...
namespace Championship_Riven
{
    class Riven
    {
        ...
        {
            if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.LastHit) && sender is Obj_AI_Turret && sender.Distance(Player.Instance) < 800 && sender.IsAlly)
            {
                ...
                if (!(args.Target is AIHeroClient))
                {
                    ...
                    var predH = Prediction.Health.GetPrediction(Minion, 500);
                    if(Minion.Health > Player.Instance.TotalAttackDamage && Minion.Health - sender.TotalAttackDamage * 1.1 <= 0 )
        ...
        {
        {
            ...
            var minions = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, Player.Instance.Position, Q.Range).Where(minion => minion != null && !minion.IsDead).ToList();
