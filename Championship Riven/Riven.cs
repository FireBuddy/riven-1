            ...
            if (Target != null)
            {
                ...
                if (CountQ == 2 && Q.IsReady() && E.IsReady() && W.IsReady() && Target.IsValidTarget(450))
                {
                    
                    {
                        ...
                        {
                           Core.DelayAction( () => Player.CastSpell(SpellSlot.W, Target.Position),200);
                           Core.DelayAction( () => E.Cast(EPos),200);
                        }
                    ...
                    foreach (var jungleMobs in jminions.Where(x => x.IsValidTarget(Player.Instance.AttackRange)))              
                            {
                                ...
                                if (jungleMobs != null)
                                {
                                        if (jungleMobs.Name =="SRU_RedMini10.1.3" || jungleMobs.Name == "SRU_BlueMini1.1.2")
                                        {
